...
import { InputNumber, Input, Select, DatePicker, Button } from 'antd'
    ...
    <div className='c-div p-2'>
        ...
        :
          <div className="fields">
            <div className="field pb-2">
              <p>意識部位カテゴリ</p>
              <Select style={{ width: 240 }} value={state.category} onChange={(value) => onInputChange(value,'category')} >
                {bodyPart.map((item:BodyPart) => (<Option key={item.id} value={item.value}>{item.part}</Option>))}
              </Select>
            </div>
            <div className="field pb-2">
              <InputNumber addonBefore="+" addonAfter="秒" value={state.useTime} onChange={(e) => {onInputChange(String(e),'useTime')}} />
            </div>
            <div className="field pb-2">
              <p>作成日</p>
              <DatePicker defaultValue={moment(state.createAt, dateFormat)} format={dateFormat} onChange={e => onInputChange(setDay(String(e)),'createAt')} />
            </div>
            <div className="field pb-2">
              <p>意識部位</p>
              <Input showCount maxLength={200} value={state.part} onChange={e => onInputChange(e.target.value,'part')} />
            </div>
            ...
            <div className="field pb-1">
              <p>目的</p>
              <TextArea showCount maxLength={400} value={state.goal} onChange={e => onInputChange(e.target.value,'goal')} />
            </div>
            <div className="field pb-1">
              {formType === 'create' ?
