          ...
          <div className="fields">
            <div className="field pb-2">
              ...
              <Space>
                ...
                <Button onClick={deleteAction} danger>削除</Button>
              </Space>
            </div>
          </div>
        :
          <div className="fields">
            <div className="field pb-2">
              <p>意識部位カテゴリ</p>
              <Select style={{ width: 240 }} onChange={(value) => onInputChange(value,'category')} >
                {bodyPart.map((item:BodyPart) => (<Option key={item.id} value={item.value}>{item.part}</Option>))}
              </Select>
            </div>
            <div className="field pb-2">
              <p>認識したカテゴリ</p>
              <Select style={{ width: 240 }} onChange={(value) => onInputChange(value,'methodId')} >
                {posts.map((item:Post) => (<Option key={item.id} value={item.id}>{item.goal}</Option>))}
              </Select>
            </div>
            <div className="field pb-2">
              <p>タイトル</p>
              <Input showCount maxLength={200} value={state.title} onChange={e => onInputChange(e.target.value,'title')} />
            </div>
            <div className="field pb-2">
              <p>リンク</p>
            ...
            <div className="field pb-1">
              ...
              <TextArea showCount maxLength={400} value={state.disc} onChange={e => onInputChange(e.target.value,'disc')} />
            </div>
            <div className="field pb-1">
              <p>結果</p>
              <TextArea showCount maxLength={400} value={state.result} onChange={e => onInputChange(e.target.value,'result')} />
            </div>
            <div className="field pb-1">
              <p>目的結果と事象の確認</p>
              <TextArea showCount maxLength={400} value={state.differenceInfo} onChange={e => onInputChange(e.target.value,'differenceInfo')} />
            </div>
            <div className="field pb-1">
              <p>達成率</p>
              <Slider value={Number(state.goalRate)} tipFormatter={(e) => formatter(e!)} onChange={e => onInputChange(String(e),'goalRate')} />
            </div>
            <div className="field pb-1">
              <p>目的達成数</p>
              <InputNumber addonBefore="+" addonAfter="個" value={state.goalScore} onChange={(e) => {onInputChange(String(e),'goalScore')}} />
            </div>
            <div className="field pb-1">
              <p>実行数</p>
              <InputNumber addonBefore="+" addonAfter="個" value={state.playScore} onChange={(e) => {onInputChange(String(e),'playScore')}} />
            </div> 
            <div className="field pb-1">
              {formType === 'create' ?
