...
function ContentPattern() {
  ...
  const AddItem = (type: string) => {
    if (type === 'issue') {
      issuesSet([
        ...issues,
        { id: issues.length + 1, text: '', itemId: -1, view: true },
      ])
    } else {
      goalSet([
        ...goal,
        { id: goal.length + 1, goal: '', itemId: -1, view: true },
      ])
    }
  }
                  ...
                  <Input
                    ...
                    addonAfter={
                      <Button
                        shape="circle"
                        danger
                        onClick={() => deleteItem('issue', item.id)}
                        icon={<DeleteOutlined />}
                      />
                    }
                    prefix={<FormOutlined />}
                  ...
                  <Input
                    ...
                    addonAfter={
                      <Button
                        shape="circle"
                        danger
                        onClick={() => deleteItem('gola', item.id)}
                        icon={<DeleteOutlined />}
                      />
                    }
                    prefix={<FormOutlined />}
