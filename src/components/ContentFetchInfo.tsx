...
function ContentFetchInfo(){
  ...
  return (
    <section className='c-section p-2'>
      <Row gutter={16}>
        {post.map((item:Post) => <Col span={8} key={item.id}>
          <Card
            title={item.goal}
            bordered={false}
            style={{ marginBottom: 16 }}
            actions={[
              <Button onClick={() => viewAction(item.id!)}><FundViewOutlined /></Button>,
              <Button onClick={() => editAction(item.id!)}><EditOutlined /></Button>,
              <Button danger onClick={() => deleteAction(item.id!)}><DeleteOutlined /></Button>,
            ]}
          >
            <p>{item.disc}</p>
            <p>{item.createAt}</p>
            <p><Tag color="#108ee9">{setLabel(item.category)}</Tag></p>
          </Card>
        </Col>)}
      </Row>
      <Modal
